...
def train_CrossEntropy(args, model, device, train_loader, optimizer, epoch, status):
    ...
    # update soft labels
    train_loader.dataset.update_labels(results) # 这里才是unlableled_index用到的地方

    return train_loss.avg, top5.avg, top1.avg, batch_time.sum, count_total


def inference(model, device, test_loader, loss):
    model.eval()
    correct = 0
    with torch.no_grad():
    ...
    wandb.log({"Test Accuracy": 100. * correct / len(test_loader.dataset),  "Loss": loss})
