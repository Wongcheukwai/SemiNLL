...
import torch
import codecs
from mnist_utils import noisify
...
class MNIST(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
                 noise_type=None, index=None, noise_rate=0.2, random_state=0):
        ...
        if self.train:
            self.train_data, self.train_labels = torch.load(
                os.path.join(self.root, self.processed_folder, self.training_file)) # 60000,28,28   60000

            if noise_type != 'clean':
                self.train_labels=np.asarray([[self.train_labels[i]] for i in range(len(self.train_labels))]) # 变成array
                ...
                self.train_noisy_labels=[i[0] for i in self.train_noisy_labels]# list 60000
                _train_labels=[i[0] for i in self.train_labels] # list 60000
                ...
                for i in range(len(self.train_noisy_labels)):
                    self.soft_labels[i][self.train_noisy_labels[i]] = 1
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            if self.noise_type != 'clean':
                img, target, soft_labels, unlabeled_soft_labels, unlabeled_labels = self.train_data[index], self.train_noisy_labels[index],\
                                                                                    self.soft_labels[index], self.unlabeled_soft_labels[index],\
