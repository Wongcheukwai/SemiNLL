...
def main(args):
    ...
    for iter in range(100):
        #early_stopping = EarlyStopping(patience=20, verbose=True, delta=0.1, model_path=model_path, optimizer_path=optimizer_path)
        ...
        if args.clean_method == "mixmatch":
            ...
            for h in unlabeled_indexes:
                if corrupted_labels[h] in predicted[h]:
                    labeled_idx_1.append(h)
                    if clean_labels[h] in predicted[h]:
                        count_true_1 += 1
                else:
