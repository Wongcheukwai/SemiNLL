...
from dataset.cifar100_v import get_dataset
...
def main(args):
    ...
    # 定义checkpoint存哪里
    total_path = '/home/zhuwang/Data/Downloads/pseudo/cifar100/checkpoint_100net'
    if args.status == 'run':
    ...
    if args.status == 'run':
        ...
        stats_log = open('./100net/%s_%.1f_%s_%s' % (args.dataset, args.r, args.noise_mode, default_exp_dir) + '_stats.txt','w')
    elif args.status == 'debug':
        # when debug
        stats_log = open('./100net/%s_%.1f_%s' % (args.dataset, args.r, args.noise_mode) + '_stats.txt', 'w')
