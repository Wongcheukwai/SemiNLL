...
from dataset.cifar10_v import get_dataset
...
def parse_args():
    ...
    parser.add_argument('--labeled_samples', type=int, default=50000, help='Number of labeled samples(whole dataset)')
...
def main(args):
    ...
    # 定义checkpoint存哪里
    total_path = '/home/zhuwang/Data/Downloads/pseudo/cifar10/checkpoint_1net'
    if args.status == 'run':
    ...
    if args.status == 'run':
        ...
        stats_log = open('./1net/%s_%.1f_%s_%s' % (args.dataset, args.r, args.noise_mode, default_exp_dir) + '_stats.txt','w')
    elif args.status == 'debug':
        # when debug
        stats_log = open('./1net/%s_%.1f_%s' % (args.dataset, args.r, args.noise_mode) + '_stats.txt', 'w')
    ...
    for iter in range(500):
        ...
        for epoch in range(1, 2):#这里epoch只能为1
            start = time.time()
            # train for one epoch
            loss_per_epoch_train, \
            top_5_train_ac, \
