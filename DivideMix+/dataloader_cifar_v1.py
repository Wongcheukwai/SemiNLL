...
class cifar_dataset(Dataset): 
    ...
    def __getitem__(self, index):
        if self.mode=='labeled':
            ...
            img3 = transform_test(img) # this is test
            return img1, img2, target, img3, index
...
class cifar_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform_train, mode="labeled", noise_file=self.noise_file, log=self.log)
            trainloader = DataLoader(
                dataset=labeled_dataset, 
                batch_size=self.batch_size,
                shuffle=True,
                num_workers=self.num_workers)
            return trainloader, labeled_dataset.clean, labeled_dataset.noise_label, labeled_dataset.train_label
            ...
            eval_loader = DataLoader(
                ...
                shuffle=False,
                num_workers=self.num_workers)          
            return eval_loader, eval_dataset.noise_label, eval_dataset.true_set